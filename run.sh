...
KD_TEMPERATURE=4.0              # Temperature for KD softmax
TEACHER_MODEL=""                # Will be set based on training target
  ...
  if is_lora; then
    ...
    merge_lora() {
      ...
      for RANK in "${lora_ranks[@]}"; do
        for MAX in "${samples[@]}"; do
          local K; K="$(format_k "$MAX")"
          local ADAP="$SRC_ROOT/${SRC_TAG_WITH_SUFFIX}-${K}-lora-rank${RANK}-${LR_TAG}-${suffix_name}"
